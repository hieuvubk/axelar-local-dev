/* Autogenerated file. Do not edit manually. */
/* tslint:disable */
/* eslint-disable */
import {
  Signer,
  utils,
  Contract,
  ContractFactory,
  BytesLike,
  Overrides,
} from "ethers";
import type { Provider, TransactionRequest } from "@ethersproject/providers";
import type { PromiseOrValue } from "../../../../common";
import type {
  DepositReceiver,
  DepositReceiverInterface,
} from "../../../../axelar-cgp-solidity/contracts/deposit-service/DepositReceiver";

const _abi = [
  {
    inputs: [
      {
        internalType: "bytes",
        name: "delegateData",
        type: "bytes",
      },
      {
        internalType: "address",
        name: "refundAddress",
        type: "address",
      },
    ],
    stateMutability: "nonpayable",
    type: "constructor",
  },
  {
    stateMutability: "payable",
    type: "receive",
  },
] as const;

const _bytecode =
  "";

type DepositReceiverConstructorParams =
  | [signer?: Signer]
  | ConstructorParameters<typeof ContractFactory>;

const isSuperArgs = (
  xs: DepositReceiverConstructorParams
): xs is ConstructorParameters<typeof ContractFactory> => xs.length > 1;

export class DepositReceiver__factory extends ContractFactory {
  constructor(...args: DepositReceiverConstructorParams) {
    if (isSuperArgs(args)) {
      super(...args);
    } else {
      super(_abi, _bytecode, args[0]);
    }
  }

  override deploy(
    delegateData: PromiseOrValue<BytesLike>,
    refundAddress: PromiseOrValue<string>,
    overrides?: Overrides & { from?: PromiseOrValue<string> }
  ): Promise<DepositReceiver> {
    return super.deploy(
      delegateData,
      refundAddress,
      overrides || {}
    ) as Promise<DepositReceiver>;
  }
  override getDeployTransaction(
    delegateData: PromiseOrValue<BytesLike>,
    refundAddress: PromiseOrValue<string>,
    overrides?: Overrides & { from?: PromiseOrValue<string> }
  ): TransactionRequest {
    return super.getDeployTransaction(
      delegateData,
      refundAddress,
      overrides || {}
    );
  }
  override attach(address: string): DepositReceiver {
    return super.attach(address) as DepositReceiver;
  }
  override connect(signer: Signer): DepositReceiver__factory {
    return super.connect(signer) as DepositReceiver__factory;
  }

  static readonly bytecode = _bytecode;
  static readonly abi = _abi;
  static createInterface(): DepositReceiverInterface {
    return new utils.Interface(_abi) as DepositReceiverInterface;
  }
  static connect(
    address: string,
    signerOrProvider: Signer | Provider
  ): DepositReceiver {
    return new Contract(address, _abi, signerOrProvider) as DepositReceiver;
  }
}
